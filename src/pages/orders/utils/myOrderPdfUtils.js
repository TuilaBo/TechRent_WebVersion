/**
 * PDF generation utilities for MyOrders
...
/**
 * Build printable HTML for contract PDF
 */
export function buildPrintableHtml(detail, customer, kyc) {
    ...
    const bankInfoHtml = bankInfo.length > 0
    ...
    const partyBSigned = status === "ACTIVE";
    const partyASigned = status === "PENDING_SIGNATURE" || status === "ACTIVE";
      ...
      <section style="display:flex;justify-content:space-between;gap:24px;margin-top:28px">
        <div style="flex:1;text-align:center">
          ...
          <div style="height:72px;display:flex;align-items:center;justify-content:center">
            ${partyBSigned ? '<div style="font-size:48px;color:#16a34a;line-height:1">✓</div>' : ""}
          </div>
          <div>
            ${partyBSigned ? `<div style="color:#000;font-weight:600">${customerName}</div>` : "(Ký, ghi rõ họ tên)"}
          </div>
        </div>
        <div style="flex:1;text-align:center">
          <div><b>ĐẠI DIỆN BÊN A</b></div>
          <div style="height:72px;display:flex;align-items:center;justify-content:center">
            ${partyASigned ? '<div style="font-size:48px;color:#16a34a;line-height:1">✓</div>' : ""}
          </div>
          <div>
            ${partyASigned ? '<div style="color:#000;font-weight:600">CÔNG TY TECHRENT</div>' : "(Ký, ghi rõ họ tên)"}
          </div>
        </div>
      </section>
    </div>
...
/**
 * Convert DOM element to PDF blob
 */
