// src/lib/contractPrintUtils.js
// Các tiện ích dùng để kết xuất hợp đồng & biên bản ra HTML/PDF

import jsPDF from "jspdf";
import html2canvas from "html2canvas";

// ---------- Helpers định dạng số / tiền ----------
...
// ---------- Helpers xử lý HTML tiền tệ & layout ----------
...
function formatDateLabelsInHtml(html = "") {
  if (!html || typeof html !== "string") return html;
  const SEP = String.raw`(?:\s|<\/?[^>]+>)*`;
  html = html.replace(new RegExp(`(Ngày${SEP}bắt${SEP}đầu)${SEP}:`, "gi"), "Ngày bắt đầu thuê:");
  html = html.replace(new RegExp(`(Ngày${SEP}kết${SEP}thúc)${SEP}:`, "gi"), "Ngày kết thúc thuê:");
  return html;
}
...
export const NATIONAL_HEADER_HTML = `
...
export function augmentContractContent(detail) {
  if (!detail) return detail;
  const originalBase = String(detail.contentHtml || detail.contractContent || "");
  const mergedHtml = originalBase + EXTRA_CONTRACT_HTML;
      ...
      <div style="
        ...
        <section style="display:flex;justify-content:space-between;gap:24px;margin-top:28px">
          <div style="flex:1;text-align:center">
            ...
            <div style="height:72px;display:flex;align-items:center;justify-content:center"></div>
